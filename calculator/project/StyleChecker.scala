...
object StyleChecker {
  ...
  class CustomTextOutput[T <: FileSpec](stream: PrintStream) extends Output[T] {
    ...
    private val messageHelper = new MessageHelper(
      ConfigFactory.load(getClass.getClassLoader.getParent)
    )
    ...
    private def report(line: Option[Int],
                       column: Option[Int],
                       level: String,
                       message: String) {
    ...
    private def pos(line: Option[Int], column: Option[Int]): String =
      line match {
  ...
  def assess(sources: Seq[File], styleSheetPath: String): (String, Int) = {
    ...
    val outputResult =
      new CustomTextOutput(new PrintStream(output)).output(messages)
